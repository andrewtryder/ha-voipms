...
async def test_sensors(hass: HomeAssistant, mock_zeep_client) -> None:
    ...
    mock_zeep_client.service.getBalance.return_value = {"status": "success", "balance": 42.50}
    ...
    balance_state = hass.states.get(f"sensor.voip_ms_account_balance")
    assert balance_state is not None
    assert balance_state.state == "42.5"

    # Test Inbound Calls Sensor
    inbound_state = hass.states.get(f"sensor.voip_ms_inbound_calls_24h")
    assert inbound_state is not None
    assert inbound_state.state == "2"

    # Test Outbound Calls Sensor
    outbound_state = hass.states.get(f"sensor.voip_ms_outbound_calls_24h")
