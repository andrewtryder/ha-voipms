    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        async with session.get(f"{HA_URL}/api/config/config_entries/entry") as resp:
            ...
            if voipms_entry["state"] != "loaded":
                print(f"Error: VoIP.ms config entry is not loaded. State: {voipms_entry['state']}")
                sys.exit(1)
            else:
        ...
        async with session.get(f"{HA_URL}/api/states") as resp:
            ...
            for state in states:
                ...
                if "voipms" in entity_id or "voicemail" in entity_id or "balance" in entity_id:
                    summary.append({
                        "entity_id": entity_id,
                        "state": state["state"]
                    })
            ...
            for k, found in found_keywords.items():
                if not found:
                    print(f"Warning: Expected entity related to '{k}' not found. Entities found: {[e['entity_id'] for e in summary]}")
        ...
        async with session.get(f"{HA_URL}/api/services") as resp:
            ...
            voipms_services = next((s for s in services if s["domain"] == "voipms"), None)
                ...
                async with session.get(f"{HA_URL}/api/config/config_entries/entry") as resp:
                    if resp.status == 200:
                        ...
                        if voipms_entry and voipms_entry["state"] == "loaded":
                            print("Success: VoIP.ms config entry remained LOADED after restart.")
                            break
            except Exception:
                pass
            await asyncio.sleep(2)
        else:
            print("Error: Home Assistant did not come back with a LOADED voipms entry after restart.")
            sys.exit(1)
